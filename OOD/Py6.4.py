def find_paths(maze, y, x, path):
    ...
    if (find_paths(maze, y+1, x, path) or
        find_paths(maze, y-1, x, path) or
        find_paths(maze, y, x+1, path) or
        find_paths(maze, y, x-1, path)):
        return True
...
def the_maze(maze):
    ...
    path = []
    if find_paths(maze, start_x, start_y, path):
