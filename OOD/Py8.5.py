...
def check_binary_search_tree_(node, min_val=0, max_val=100):
    if node is None:
        return True
    ...
    return (check_binary_search_tree_(node.left, min_val, node.data-1 ) and
            check_binary_search_tree_(node.right, node.data+1, max_val))
